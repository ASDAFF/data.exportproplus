...
class data_exportproplus extends CModule
{
    const MODULE_ID = 'data.exportproplus';
    var $MODULE_ID = 'data.exportproplus';
    var $MODULE_VERSION;
    ...
    {
        $this->_269256199 = toLower(preg_replace('#^data\.(.*?)$#i', '$1', $this->MODULE_ID));
        ...
        $this->MODULE_NAME = GetMessage('DATA_EXPORTPROPLUS_MODULE_NAME');
        $this->MODULE_DESCRIPTION = GetMessage('DATA_EXPORTPROPLUS_MODULE_DESC');
        $this->PARTNER_NAME = GetMessage('DATA_EXPORTPROPLUS_PARTNER_NAME');
        $this->PARTNER_URI = GetMessage('DATA_EXPORTPROPLUS_PARTNER_URI');
    ...
    function DoInstall()
    {
        ...
        if (!\Bitrix\Main\Loader::includeModule('data.core')) {
            $APPLICATION->ThrowException(getMessage('DATA_' . $this->_867037812 . '_NO_CORE'));
            return false;
        }
        if (!CheckVersion(PHP_VERSION, '5.4.0')) {
            $APPLICATION->ThrowException(GetMessage('DATA_' . $this->_867037812 . '_PHP_REQUIRE'));
            return false;
        }
        $this->reset();
        CJSCore::Init('jquery');
        if (!isset($step) || ($step < 1)) {
            $APPLICATION->IncludeAdminFile(GetMessage('DATA_' . $this->_867037812 . '_RECOMMENDED'), $_SERVER['DOCUMENT_ROOT'] . '/bitrix/modules/' . $this->MODULE_ID . '/install/step.php');
            ...
            if (\Bitrix\Main\Loader::includeModule('data.core') && class_exists('\Data\Core\Helper')) {
                \Data\Core\Helper::startBitrixCloudMonitoring('admin@data.ru');
                $_2130876983 = \Data\Core\Cli::getPhpPath();
            ...
            foreach ($this->_997989597 as $_84438248 => $_1376128280) {
                $_1675345452->add(['SITE_ID' => $_84438248, 'CONDITION' => '#^/data.' . $this->_269256199 . '/(.*)#', 'PATH' => '/data.' . $this->_269256199 . '/index.php', 'RULE' => 'path=$1',]);
            }
        ...
        if (is_dir($_599757263 = $_SERVER['DOCUMENT_ROOT'] . '/bitrix/modules/' . $this->MODULE_ID . '/admin')) {
            if ($_139523034 = opendir($_599757263)) {
                while (false !== ($_196259126 = readdir($_139523034))) {
                    ...
                    file_put_contents($_SERVER['DOCUMENT_ROOT'] . '/bitrix/admin/data_' . $this->_269256199 . '_' . $_196259126, '' . $this->MODULE_ID . '/admin/' . $_196259126 . '");?>');
                }
                closedir($_139523034);
            }
        }
    ...
    function InstallDB($_261457188 = [])
    {
        ...
        if (\Bitrix\Main\Loader::includeModule('security')) {
            ...
            $_232453927[] = ['MASK' => '/bitrix/admin/data_' . $this->_269256199 . '_edit.php', 'SITE_ID' => '',];
            $_232453927[] = ['MASK' => '/bitrix/admin/data_' . $this->_269256199 . '_new_edit.php*', 'SITE_ID' => '',];
        ...
        if ($this->_945354433 === false && \Bitrix\Main\Loader::includeModule('data.core')) {
            $this->_945354433 = \Data\Core\Helper::runCoreSqlBatch('export/install.sql', $this->MODULE_ID);
        }
    ...
    {
        RegisterModuleDependences('main', 'OnBuildGlobalMenu', $this->MODULE_ID, 'CData' . $this->_97845374 . 'Menu', 'OnBuildGlobalMenu');
        RegisterModuleDependences('main', 'OnEndBufferContent', $this->MODULE_ID, 'CData' . $this->_97845374 . 'Remarketing', 'OnEndBufferContent');
        RegisterModuleDependences('main', 'OnEndBufferContent', $this->MODULE_ID, 'CData' . $this->_97845374 . 'GoogleTagManager', 'OnEndBufferContent');
        RegisterModuleDependences($this->MODULE_ID, 'OnCondCatControlBuildList', $this->MODULE_ID, 'CDataCatalogCondCtrlGroup', 'GetControlDescr', 100);
        RegisterModuleDependences($this->MODULE_ID, 'OnCondCatControlBuildList', $this->MODULE_ID, 'CDataCatalogCondCtrlIBlockFields', 'GetControlDescr', 200);
        RegisterModuleDependences($this->MODULE_ID, 'OnCondCatControlBuildList', $this->MODULE_ID, 'CDataCatalogCondCtrlIBlockProps', 'GetControlDescr', 300);
        return true;
    }
    ...
    function DoUninstall()
    {
        ...
        $GLOBALS['DATA_MODULE_NAME'] = $this->MODULE_NAME;
        if ($_REQUEST['step'] < 2) {
            $APPLICATION->IncludeAdminFile(GetMessage('data.' . $this->_269256199 . '_UNINSTALL_TITLE'), $_SERVER['DOCUMENT_ROOT'] . '/bitrix/modules/' . $this->MODULE_ID . '/install/uninst_form.php');
            ...
            $this->reset();
            $APPLICATION->IncludeAdminFile(GetMessage('data.' . $this->_269256199 . '_UNINSTALL_TITLE'), $_SERVER['DOCUMENT_ROOT'] . '/bitrix/modules/' . $this->MODULE_ID . '/install/uninst_mail.php');
        }
    }
    ...
    {
        UnRegisterModuleDependences('main', 'OnBuildGlobalMenu', $this->MODULE_ID, 'CData' . $this->_97845374 . 'Menu', 'OnBuildGlobalMenu');
        UnRegisterModuleDependences('main', 'OnEndBufferContent', $this->MODULE_ID, 'CData' . $this->_97845374 . 'Remarketing', 'OnEndBufferContent');
        UnRegisterModuleDependences('main', 'OnEndBufferContent', $this->MODULE_ID, 'CData' . $this->_97845374 . 'GoogleTagManager', 'OnEndBufferContent');
        UnRegisterModuleDependences($this->MODULE_ID, 'OnCondCatControlBuildList', $this->MODULE_ID, 'CDataCatalogCondCtrlGroup', 'GetControlDescr');
        UnRegisterModuleDependences($this->MODULE_ID, 'OnCondCatControlBuildList', $this->MODULE_ID, 'CDataCatalogCondCtrlIBlockFields', 'GetControlDescr');
        UnRegisterModuleDependences($this->MODULE_ID, 'OnCondCatControlBuildList', $this->MODULE_ID, 'CDataCatalogCondCtrlIBlockProps', 'GetControlDescr');
        return true;
    }
    ...
    function UnInstallFiles()
    {
        ...
        if ($_ENV['COMPUTERNAME'] != 'BX') {
            DeleteDirFilesEx('/bitrix/js/data.' . $this->_269256199 . '/');
            DeleteDirFilesEx('/bitrix/gadgets/data/' . $this->_269256199 . '/');
            DeleteDirFilesEx('/bitrix/tools/data.' . $this->_269256199 . '/');
            DeleteDirFilesEx('/bitrix/themes/.default/data/' . $this->_269256199 . '/');
            DeleteDirFilesEx('/bitrix/themes/.default/data/' . $this->_269256199 . '/');
            DeleteDirFilesEx('/bitrix/themes/.default/data.' . $this->_269256199 . '.css');
            DeleteDirFilesEx('/upload/data_' . $this->_269256199 . '/');
    ...
    function UninstallDB($_261457188 = [])
    {
        ...
        if (\Bitrix\Main\Loader::includeModule('security')) {
            $_1514107607 = \CSecurityFilterMask::GetList();
            $_836166561 = ['/bitrix/admin/data_' . $this->_269256199 . '_edit.php', '/bitrix/admin/data_' . $this->_269256199 . '_new_edit.php*'];
            $_232453927 = [];
            while ($_264167126 = $_1514107607->Fetch()) {
        ...
        if (\Bitrix\Main\Loader::includeModule('data.core')) {
            ...
            \Data\Core\Helper::runCoreSqlBatch('export/uninstall.sql', $this->MODULE_ID);
        }
        return true;
    }
} ?>
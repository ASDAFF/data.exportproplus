...
class CAcritExportproplusLog{
    public $logFilename = "/upload/acrit.exportproplus/";
    private $profileId;
    private $session;
    ...
    public static function AcritDump( $dumpData, $clear = FALSE, $depth = 0 ){
        $fileName = "acrit_exportproplus_dump.txt";
    ...
    public function Init( $profile ){
        $sessionData = AcritExportproplusSession::GetSession( $profile["ID"] );
        ...
        $sEmptyFileContent = ( $profile["TYPE"] == "ozon_api" ) ? GetMessage( "ACRIT_EXPORTPROPLUS_REQUIRED_FIELD_OZON_ERRORS_DESC" ) : "";

        file_put_contents( $_SERVER["DOCUMENT_ROOT"].$this->logFilename, $sEmptyFileContent );

        AcritExportproplusSession::SetSession( $profile["ID"], $sessionData );
    }

    public function IncIblcok(){
        $sessionData = AcritExportproplusSession::GetSession( $this->profileId );
        $sessionData["EXPORTPROPLUS"]["LOG"][$this->profileId]["IBLOCK"]++;
        AcritExportproplusSession::SetSession( $this->profileId, $sessionData );
    }
    ...
    public function GetLog( $profileID, $bSendEmailReport = true, $arSupportData = false ){
        ...
        $arSessionData = AcritExportproplusSession::GetAllSession( $profileID );
        $sessionData = array();
        ...
        if( $bSendEmailReport ){
            ...
            if( check_email( $arProfile["SEND_LOG_EMAIL"] ) ){
                ...
                $statVal = "";
                if( !$exportTimeStamp ){
                    $statVal = GetMessage( "ACRIT_EXPORTPROPLUS_LOG_STAT_GENERATE" );
                }
                elseif( $exportTimeStamp < $profileTimeStamp ){
                    $statVal = GetMessage( "ACRIT_EXPORTPROPLUS_LOG_STAT_REGENERATE" );
                }
                else{
                    if( $arProfile["SETUP"]["TYPE_RUN"] == "comp" ){
                        $statVal = GetMessage( "ACRIT_EXPORTPROPLUS_LOG_STAT_FINISHED" );
                    }
                    else{
                        $maxCronProducts = 0;
                        ...
                        if( !$maxCronProducts ){
                            ...
                            if( $unloandedPercent >= 100 ){
                                ...
                                $statVal = GetMessage( "ACRIT_EXPORTPROPLUS_LOG_STAT_IN_PROCESS_BEGIN" )." ".$arProfile["UNLOADED_OFFERS_CORRECT"]." ".GetMessage( "ACRIT_EXPORTPROPLUS_LOG_STAT_IN_PROCESS_END" ).$unloandedPercent;
                            }
                        }
                    }
                }
                ...
                $messageBlock = GetMessage( "ACRIT_LOG_SEND_OFFERS" ).$sessionData["EXPORTPROPLUS"]["LOG"][$profileID]["PRODUCTS"]."\n".
                GetMessage( "ACRIT_LOG_SEND_OFFERS_TERM" ).$sessionData["EXPORTPROPLUS"]["LOG"][$profileID]["PRODUCTS_EXPORT"]."\n".
                GetMessage( "ACRIT_LOG_SEND_OFFERS_ERROR" ).$sessionData["EXPORTPROPLUS"]["LOG"][$profileID]["PRODUCTS_ERROR"]."\n".
                GetMessage( "ACRIT_LOG_SEND_DATE" ).$sessionData["EXPORTPROPLUS"]["LOG"][$profileID]["LAST_START_EXPORT"]."\n\n";

                if( is_array( $arSupportData ) && !empty( $arSupportData ) ){
                    $messageBlock .= GetMessage( "ACRIT_EXPORTPROPLUS_LOG_STAT_SUPPORT_DATA" )."\n";
                ...
                if( strlen( $statVal ) > 0 ){
                    $messageBlock .= GetMessage( "ACRIT_EXPORTPROPLUS_LOG_STAT" ).$statVal."\n\n";
                }

                $messageBlock .= GetMessage( "ACRIT_LOG_PROFILE" ).$arProfile["SITE_PROTOCOL"]."://".$arProfile["DOMAIN_NAME"]."/bitrix/admin/acrit_exportproplus_edit.php?ID=".$profileID."\n".
                GetMessage( "ACRIT_LOG_SEND_FILE" ).$arProfile["SITE_PROTOCOL"]."://".$arProfile["DOMAIN_NAME"].$sessionData["EXPORTPROPLUS"]["LOG"][$profileID]["FILE"];

...
<?}
    ...
    if( !is_array( $t ) || !isset( $t["js"] ) || !file_exists( $DOCUMENT_ROOT.$t["js"] ) ){
        $APPLICATION->ThrowException( GetMessage( "ACRIT_EXPORTPROPLUS_JQUERY_REQUIRE" ) );
    }
    ...
    $cData = new CExportproplusProfileDB();
    ...
    while( $arRes = $rsData->NavNext( true, "f_" ) ){
	    ...
	    $row->AddViewField( "NAME", '<a href="acrit_exportproplus_edit.php?ID='.$f_ID."&amp;lang=".LANG.'" title="'.GetMessage( "parser_act_edit" ).'">'.$f_NAME."</a>" );
	    ...
	    if( $POST_RIGHT == "W" ){
		    $arActions[] = array(
			    ...
			    "ACTION" => $lAdmin->ActionRedirect( "acrit_exportproplus_edit.php?ID=".$f_ID )
		    );
	    }
	    ...
	    if( $POST_RIGHT == "W" ){
		    $arActions[] = array(
			    ...
			    "ACTION" => $lAdmin->ActionRedirect( "acrit_exportproplus_edit.php?copy=$f_ID&ID=$f_ID" )
		    );
	    }
        ...
        if( file_exists( $_SERVER["DOCUMENT_ROOT"]."/bitrix/tools/$moduleId/export_{$arRes["ID"]}_run.lock" ) ){
            if( $POST_RIGHT == "W" ){
                $arActions[] = array(
                    ...
                    "ACTION" => "UnlockExportExpress( ".$arRes["ID"]." );".$lAdmin->ActionRedirect( "acrit_exportproplus_list.php" )
                );
            }
        }
        ...
        if( !file_exists( $_SERVER["DOCUMENT_ROOT"]."/bitrix/tools/$moduleId/export_{$arRes["ID"]}_run.lock" )
            && ( $f_SETUP["TYPE_RUN"] == "comp" ) ){
                if( $POST_RIGHT == "W" ){
                    $runRow = "/bitrix/tools/$moduleId/acrit_exportproplus.php?ID=".$arRes["ID"];
        ...
        if( $POST_RIGHT == "W" ){
            $arActions[] = array(
                ...
                "ACTION" => $lAdmin->ActionRedirect( "acrit_exportproplus_export.php?URL_DATA_FILE_EXPORT=/upload/acrit_exportproplus_dump_".time().".txt&export_import=export&step=2&ID=$f_ID" )
            );
        }
    ...
    $aContext = array(
	    ...
	    array(
		    "TEXT" => GetMessage( "ACRIT_EXPORTPROPLUS_NEW_MIGRATE" ),
		    "LINK" => "acrit_exportproplus_new_migrator.php?lang=".LANG,
		    "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_NEW_MIGRATE" ),
	    ),
    );
    ...
    /*if( CExportproplusInformer::CheckCRMIntergation() ){
        echo BeginNote();

...
if( ( ( $arProfile["TYPE"] == "ebay_1" ) || ( $arProfile["TYPE"] == "ebay_2" ) ) && !empty( $categories ) ){
    $obMarketCategory = new CExportproplusMarketEbayDB();
...
<tr align="center">
    <td colspan="2">
        <?=BeginNote();?>
        <?=GetMessage( "ACRIT_EXPORTPROPLUS_MARKET_CATEGORY_EBAY_DESCRIPTION" )?>
        <?=EndNote();?>
    </td>
</tr>
<tr>
	<td colspan="2" id="market_category_data_ebay">
			...
			<?foreach( $categories as $cat ){?>
				<tr>
					...
					<td width="60%">
						...
						<span class="field-edit" onclick="ShowMarketCategoryList( <?=$cat["ID"]?>, 'market_category_list_ebay' )" style="cursor: pointer; background: #9ec710 !important;" title="<?=GetMessage( "ACRIT_EXPORTPROPLUS_MARKET_CATEGORY_DATA_SELECT_SECTION" );?>"></span>
					</td>
				</tr>
			<?}?>
		</table>
		<div id="market_category_list_ebay" style="display: none">
            ...
            <input onkeyup="FilterMarketCategoryList( this, 'market_category_list_ebay' )" placeholder="<?=GetMessage( "ACRIT_EXPORTPROPLUS_MARKET_CATEGORY_DATA_WINDOW_PLACEHOLDER" );?>">

...
if( $moduleStatus == MODULE_DEMO_EXPIRED ){
    $buyLicenceUrl = "http://www.acrit-studio.ru/market/rabota-s-torgovymi-ploshchadkami/eksport-tovarov-na-torgovye-portaly/?action=BUY&id=96558";
    require_once( $_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/prolog_admin_after.php" );?>
    <div class="adm-info-message">
        <div class="acrit_note_button">
            <a href="<?=$buyLicenceUrl?>" target="_blank" class="adm-btn adm-btn-save"><?=GetMessage( "ACRIT_EXPORTPROPLUS_EXPORT_DEMOEND_BUY_LICENCE_INFO" )?></a>
        </div>
        <div class="acrit_note_text"><?=GetMessage( "ACRIT_EXPORTPROPLUS_EXPORT_DEMOEND_PERIOD_INFO" );?></div>
...
<?}
    ...
    $APPLICATION->AddHeadScript( "/bitrix/js/acrit.exportproplus/main.js" );
    $t = CJSCore::getExtInfo( "jquery" );

    if( !is_array( $t ) || !isset( $t["js"] ) || !file_exists( $DOCUMENT_ROOT.$t["js"] ) ){
        try{
            throw new SystemException( GetMessage( "ACRIT_EXPORTPROPLUS_JQUERY_REQUIRE" ) );
        }
    ...
    $profileTitle = GetMessage( "ACRIT_EXPORTPROPLUS_EDITPROFILE" ).": #".$arProfile["ID"]." ".$arProfile["NAME"];
    $APPLICATION->SetTitle( ( $ID > 0 ? $profileTitle : GetMessage( "ACRIT_EXPORTPROPLUS_ADDPROFILE" ) ) );
    if( $copy && $ID > 0 ){
        unset( $arProfile["ID"] );
    ...
    $aContext = array(
        array(
            ...
            "LINK" => "acrit_exportproplus_edit.php?lang=".LANG,
    ...
    $aMenu = array(
        array(
            "TEXT" => GetMessage( "ACRIT_EXPORTPROPLUS_LIST" ),
            "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_LIST" ),
            "LINK" => "acrit_exportproplus_list.php?lang=".LANG,
            "ICON" => "btn_list",
        ),
        array(
            "TEXT" => GetMessage( "ACRIT_EXPORTPROPLUS_SAVE" ),
            "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_SAVE" ),
            "LINK" => "javascript:$('.adm-btn-save').click()",
            "ICON" => "",
        ),
        array(
            "TEXT" => GetMessage( "ACRIT_EXPORTPROPLUS_INSTRUCTION" ),
            "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_INSTRUCTION" ),
    ...
    if( $ID > 0 ){
        ...
        $aMenu[] = array(
            "TEXT" => GetMessage( "ACRIT_EXPORTPROPLUS_ADD" ),
            "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_ADD" ),
            "LINK" => "acrit_exportproplus_edit.php?lang=".LANG,
            "ICON" => "btn_new",
        );
        $aMenu[] = array(
            "TEXT" => GetMessage( "ACRIT_EXPORTPROPLUS_COPY" ),
            "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_COPY" ),
            "LINK" => "acrit_exportproplus_edit.php?copy=$ID&ID=$ID&lang=".LANG,
            "ICON" => "btn_copy",
        );
        $aMenu[] = array(
            "TEXT" => GetMessage( "ACRIT_EXPORTPROPLUS_DEL" ),
            "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_DEL" ),
            "LINK" => "javascript:if(confirm('".GetMessage( "ACRIT_EXPORTPROPLUS_PROFILE_DELETE_CONFIRM" )."'))window.location='acrit_exportproplus_list.php?ID=".$ID."&action=delete&lang=".LANG."&".bitrix_sessid_get()."';",
            "ICON" => "btn_delete",
        );
        $aMenu[] = array( "SEPARATOR" => "Y" );
        $aMenu[] = array(
            "TEXT" => GetMessage( "ACRIT_EXPORTPROPLUS_RUN" ),
            "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_RUN" ),
            "LINK" => "/bitrix/tools/acrit.exportproplus/acrit_exportproplus.php?ID=".$ID,
						"PUBLIC" => "Y",
    ...
    function CheckFields(){
        ...
        foreach( $requiredFields as $field ){
            if( !$PROFILE[$field] ){
                try{
                    throw new SystemException( GetMessage( "ACRIT_EXPORTPROPLUS_REQUIRED_INPUT_FIELD_FAIL", array( "#CODE#" => $field, "#NAME#" => GetMessage( "ACRIT_EXPORTPROPLUS_STEP1_".$field ) ) ) );
                }
                catch( SystemException $exception ){
                    global $lastException;
                    $lastException = $exception->getMessage();
        ...
        foreach( $PROFILE["XMLDATA"] as $id => $field ){
            if( $field["REQUIRED"] == "Y" ){
                if( ( $field["TYPE"] == "field" && !$field["VALUE"] ) || ( $field["TYPE"] == "const" && !$field["CONTVALUE_TRUE"] ) || $field["TYPE"] == "none" ){
                    try{
                        throw new SystemException( GetMessage( "ACRIT_EXPORTPROPLUS_REQUIRED_FIELD_FAIL", array( "#CODE#" => $field["CODE"], "#NAME#" => $field["NAME"] ) ) );
                    }
                    catch( SystemException $exception ){
                        global $lastException;
                        $lastException = $exception->getMessage();
            ...
            if( is_array( $PROFILE["MARKET_CATEGORY"]["CATEGORY_LIST"] )
                ...
                foreach( $PROFILE["MARKET_CATEGORY"]["CATEGORY_LIST"] as $categoryItem ){
                    if( strlen( trim( $categoryItem ) ) <= 0 ){
                        try{
                            throw new SystemException( GetMessage( "ACRIT_EXPORTPROPLUS_EMPTY_MARKET_CATEGORY_FIELDS" ) );
                        }
                        catch( SystemException $exception ){
                            global $lastException;
                            $lastException = $exception->getMessage();
    ...
    if( $_SERVER["REQUEST_METHOD"] == "POST" && ( !empty( $save ) || !empty( $apply ) ) && !$bUnlockMode && check_bitrix_sessid() ){
        ...
        if( $fieldsCheck = CheckFields() ){
            ...
            if( !CAcritExportproplusTools::ProcessMarketCategoriesOnEmpty( $PROFILE["VK"]["VK_MARKET_ALBUMS"] ) ){
                $PROFILE["VK"]["VK_MARKET_ALBUMS"] = null;
            }

            if( !CAcritExportproplusTools::ProcessMarketCategoriesOnEmpty( $PROFILE["VK"]["VK_ALBUMS"] ) ){
            ...
            if( isset( $ID ) && ( intval( $ID ) > 0 ) ){
                if(
                    ...
                    if( is_array( $arIntersectAgents ) && !empty( $arIntersectAgents ) ){
                        echo BeginNote();
                        echo GetMessage( "ACRIT_EXPORTPROPLUS_PROFILE_HAS_INTERSECT_AGENTS" );
                        foreach( $arIntersectAgents as $arIntersectAgentsItem ){
                            echo $arIntersectAgentsItem["NAME"].": ".$arIntersectAgentsItem["NEXT_EXEC"]."<br/>";
                        }
                        echo GetMessage( "ACRIT_EXPORTPROPLUS_PROFILE_HAS_INTERSECT_AGENTS_END" );
                        echo EndNote();
                    }
                }
            }
            ...
            switch( $PROFILE["TYPE"] ){
                ...
                case "ebay_2":
                case "ebay_mp30":
                    if( !CAcritExportproplusTools::ProcessMarketCategoriesOnEmpty( $PROFILE["MARKET_CATEGORY"]["EBAY"]["CATEGORY_LIST"] ) ){
                        $PROFILE["MARKET_CATEGORY"] = null;
                    }
                    break;
                case "ozon":
                case "ozon_api":
                    if( !CAcritExportproplusTools::ProcessMarketCategoriesOnEmpty( $PROFILE["MARKET_CATEGORY"]["OZON"]["CATEGORY_LIST"] ) ){
                        $PROFILE["MARKET_CATEGORY"] = null;
                    }
                    break;
                case "vk_trade":
                    if( !CAcritExportproplusTools::ProcessMarketCategoriesOnEmpty( $PROFILE["MARKET_CATEGORY"]["VK"]["CATEGORY_LIST"] ) ){
                        $PROFILE["MARKET_CATEGORY"] = null;
                    }
                    $PROFILE["VK"]["VK_RELATIONS"] = $arProfile["VK"]["VK_RELATIONS"];
                    break;
                default:
                    if( !CAcritExportproplusTools::ProcessMarketCategoriesOnEmpty( $PROFILE["MARKET_CATEGORY"]["CATEGORY_LIST"] ) ){
                        $PROFILE["MARKET_CATEGORY"] = null;
                    }
                    break;
            }
            ...
            if( is_array( $PROFILE["MARKET_CATEGORY"]["CATEGORY_LIST"] ) && !empty( $PROFILE["MARKET_CATEGORY"]["CATEGORY_LIST"] ) ){
                array_walk( $PROFILE["MARKET_CATEGORY"]["CATEGORY_LIST"], "CAcritExportproplusStringProcess::AcritArrayTrimFunc" );
            }
            ...
            if( $ID ){
                ...
                if( ( $arExportFilePath[1] == $moduleId ) && ( strlen( $arExportFilePath[2] ) > 0 ) ){
                    ...
                    CAcritExportproplusTools::SetProfileProtect( $arExportFilePath[2], $bHasProtect );

                    CAcritExportproplusTools::SetProfilePassword(
                        array(
            ...
            if( $save ){
                LocalRedirect( "acrit_exportproplus_list.php" );
                die();
            }
            else{
                ...
                if( !isset( $arQuery["ID"] ) || empty( $arQuery["ID"] ) ){
                    $arQuery["ID"] = $ID;
                    LocalRedirect( "acrit_exportproplus_edit.php?".http_build_query( $arQuery ) );
                    die();
                }
            }
        }
    ...
    if( !isset( $_REQUEST["ajax"] ) && !isset( $_REQUEST["ib"] ) && !isset( $_REQUEST["ajax_start"] ) && !isset( $_REQUEST["ajax_count"] ) && !isset( $_POST["auth"] ) ){
        $arTabs = array(
            array(
                "DIV" => "step3",
                "TAB" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB3" ),
                "ICON" => "main_user_edit",
                "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB3" )
            ),
        );

        $arTabs[] = array( "DIV" => "step16", "TAB" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB16" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB16" ) );
        $arTabs[] = array( "DIV" => "step17", "TAB" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB17" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB17" ) );
        $arTabs[] = array( "DIV" => "step18", "TAB" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB18" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB18" ) );
        $arTabs[] = array( "DIV" => "step21", "TAB" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB21" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB21" ) );
        $arTabs[] = array( "DIV" => "step22", "TAB" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB22" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB22" ) );
        $arTabs[] = array( "DIV" => "step26", "TAB" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB26" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB26" ) );
        $arTabs[] = array( "DIV" => "step27", "TAB" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB27" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB27" ) );
        $arTabs[] = array( "DIV" => "step28", "TAB" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB28" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB28" ) );
        $arTabs[] = array( "DIV" => "step30", "TAB" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB30" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB30" ) );

        $arTabs[] = array( "DIV" => "step1", "TAB" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB1" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB1" ) );
        $arTabs[] = array( "DIV" => "step4", "TAB" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB4" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB4" ) );
        $arTabs[] = array( "DIV" => "step5", "TAB" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB5" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB5" ) );

        if( CModule::IncludeModule( "catalog" ) ){
            $arTabs[] = array( "DIV" => "step7", "TAB" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB7" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB7" ) );
        }

        $arTabs[] = array( "DIV" => "step6", "TAB" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB6" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB6" ) );

        $arTabs[] = array( "DIV" => "step12", "TAB" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB12" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB12" ) );
        $arTabs[] = array( "DIV" => "step13", "TAB" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB13" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB13" ) );
        $arTabs[] = array( "DIV" => "step10", "TAB" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB10" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB10" ) );

        $arTabs[] = array( "DIV" => "step23", "TAB" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB23" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB23" ) );
        $arTabs[] = array( "DIV" => "step24", "TAB" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB24" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB24" ) );
        $arTabs[] = array( "DIV" => "step25", "TAB" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB25" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB25" ) );
        $arTabs[] = array( "DIV" => "step29", "TAB" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB29" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB29" ) );
        $arTabs[] = array( "DIV" => "step31", "TAB" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB31" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB31" ) );

        $arTabs[] = array( "DIV" => "step9", "TAB" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB9" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB9" ) );
        $arTabs[] = array( "DIV" => "step15", "TAB" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB15" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "ACRIT_EXPORTPROPLUS_TAB15" ) );
        ...
        if( !$bHasModuleUpdates ){
            echo BeginNote();
            echo GetMessage( "ACRIT_EXPORTPROPLUS_NO_UPDATES" );
            echo EndNote();
        }
        else{
            echo BeginNote();
            echo GetMessage( "ACRIT_EXPORTPROPLUS_NEW_UPDATES" );
            echo EndNote();
        }

        $buyLicenceUrl = "http://www.acrit-studio.ru/market/rabota-s-torgovymi-ploshchadkami/eksport-tovarov-na-torgovye-portaly/?action=BUY&id=96558";
        ?>
        <div class="adm-info-message"<?if( $bDemo ){?> style="float: left;"<?}?>><?=GetMessage( "ACRIT_EXPORTPROPLUS_EXPORT_CHANGE_PROFILE_INFO" );?></div>

        <?if( $bNeedPayment ){?>
            <div class="adm-info-message" style="float: right; padding-left: 20px;">
                <div style="padding-left: 20px; float: right;">
                    <a href="<?=$needPaymentUrl?>" target="_blank" class="adm-btn adm-btn-save"><?=GetMessage( "ACRIT_EXPORTPROPLUS_EXPORT_NEED_PAYMENT_INFO" )?></a>
                </div>
                <div style="float: left; padding-top: 5px;"><?=GetMessage( "ACRIT_EXPORTPROPLUS_EXPORT_NEED_PAYMENT_INFO_TEXT" )."<b>".$timeDemoOff."</b>";?></div>
        ...
        <?if( $bDemo ){?>
            <div class="adm-info-message" style="float: right; padding-left: 20px;">
                <div style="padding-left: 20px; float: right;">
                    <a href="<?=$buyLicenceUrl?>" target="_blank" class="adm-btn adm-btn-save"><?=GetMessage( "ACRIT_EXPORTPROPLUS_EXPORT_BUY_LICENCE_INFO" )?></a>
                </div>
                <div style="float: left; padding-top: 5px;"><?=GetMessage( "ACRIT_EXPORTPROPLUS_EXPORT_DEMO_PERIOD_INFO" )."<b>".$timeDemoOff."</b>";?></div>
                <div style="clear: both;"></div>
            </div>
        <?}?>
        ...
        <?if( $arProfile["TYPE"] == "vk_trade" ){
            ...
            if( is_array( $groupAccountData["error"] ) && !empty( $groupAccountData["error"] ) ){?>
                <?=BeginNote();?>
                <?=GetMessage( "ACRIT_EXPORTPROPLUS_OP_UA_VK_ERROR_TITLE" ).$groupAccountData["error"]["error_code"].GetMessage( "ACRIT_EXPORTPROPLUS_OP_UA_VK_ERROR_MSG" ).$groupAccountData["error"]["error_msg"];?>
                <?=EndNote();?>
            <?}
        }?>
        ...
        <?// check session id?>
            ...
            $tabControl->Buttons(
                array(
                    ...
                    "back_url" => "acrit_exportproplus_list.php?lang=".LANG,
                )
            );
        ...
        <script>
            function ShowMarketCategoryList( categoryId, listContainer ){
                ...
                if( typeof MarketCategoryObject == 'undefined' ){
                    MarketCategoryObject = new BX.PopupWindow( 'my_answer', null, {
                        ...
                        titleBar: { content: BX.create( 'div', { html: '<?=GetMessage( "ACRIT_EXPORTPROPLUS_MARKET_CATEGORY_POPUP_TITLE" );?>', 'props': { 'className': 'access-title-bar' } } ) },
                        zIndex: 0,
                        offsetLeft: 0,
                        offsetTop: 0,
            ...
            function ShowFieldsList( containerPrefix, fieldId, compositeId, selectedFieldId ){
                ...
                FieldsListObject = new BX.PopupWindow( 'select_field' + containerPrefix + fieldId + ( ( compositeId != false ) ? compositeId : '' ), null, {
                    content: BX( listContainer ),
                    closeIcon: { right: '20px', top: '20px' },
                    titleBar: { content: BX.create( 'div', { html: '<?=GetMessage( "ACRIT_EXPORTPROPLUS_FIELDS_LIST_POPUP_TITLE" );?>', 'props': { 'className': 'access-title-bar' } } ) },
                    zIndex: 0,
                    offsetLeft: 0,
                    offsetTop: 0,
                    draggable: { restrict: false },
            ...
            function ShowPropertyList( obj ){
                ...
                if( typeof PropertyListObject == 'undefined' ){
                    PropertyListObject = new BX.PopupWindow( 'property_list_answer', null, {
                        ...
                        titleBar: { content: BX.create( 'span', { html: '<?=GetMessage( "ACRIT_EXPORTPROPLUS_VARIANT_CATEGORY_POPUP_TITLE" );?>', 'props': { 'className': 'access-title-bar' } } ) },
                        zIndex: 0,
                        offsetLeft: 0,
                        offsetTop: 0,
                        draggable: { restrict: false },

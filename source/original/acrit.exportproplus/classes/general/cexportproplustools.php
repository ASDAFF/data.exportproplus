...
class CAcritExportproplusTools{
    ...
    public function AcritResizeImage( $file, $width, $height, $minWidth, $minHeight ){
        $needResize = false;
        $resizeParams = array();
        if( $height < $minHeight ){
            $needResize = true;
        ...
        if( !$needResize ){
            ...
            if( !class_exists( "\Imagick" ) ){
                throw new \Exception( GetMessage( "ACRIT_EXPORTPROPLUS_IMAGE_SIZE" ).$width."*".$height.", ".GetMessage( "ACRIT_EXPORTPROPLUS_IMAGE_SIZE_LARGE_IMPOSSIBLE" ) );
            }
    ...
    public function GetSections( $arProfile ){
        $arSessionData = AcritExportproplusSession::GetAllSession( $arProfile["ID"] );
        $sessionData = array();
    ...
    public function SaveSections( $arProfile, $sections ){
        if( is_array( $sections ) ){
            $sessionData = AcritExportproplusSession::GetSession( $arProfile["ID"] );
            ...
            AcritExportproplusSession::SetSession( $arProfile["ID"], $sessionData );
        }
    }
    ...
    public function SetProfileProtect( $exportFile, $bHasProtect ){
        $cfgFileSize = filesize( $_SERVER["DOCUMENT_ROOT"]."/acrit.exportproplus/.htaccess" );
        $fp = fopen( $_SERVER["DOCUMENT_ROOT"]."/acrit.exportproplus/.htaccess", "rb" );
        ...
        if( $bHasProtect ){
            ...
            .'AuthUserFile '.$_SERVER["DOCUMENT_ROOT"].'/acrit.exportproplus/.htpasswd'.PHP_EOL
        ...
        file_put_contents( $_SERVER["DOCUMENT_ROOT"]."/acrit.exportproplus/.htaccess", $cfgData );
    }
    ...
    public function SetProfilePassword( $arExportData, $bHasProtect ){
        ...
        $arProtectRows = file( $_SERVER["DOCUMENT_ROOT"]."/acrit.exportproplus/.htpasswd" );
        ...
        file_put_contents( $_SERVER["DOCUMENT_ROOT"]."/acrit.exportproplus/.htpasswd", $cfgProtectFile );
    }
    ...
    public function HasBadCronExports( $arProfile ){
        ...
        $exportPath = ( $arPath[1] == "acrit.exportproplus" ) ? $_SERVER["DOCUMENT_ROOT"]."/upload/".$arPath[1]."/" : $_SERVER["DOCUMENT_ROOT"]."/".$arPath[1]."/";
    ...
    public function ClearExportSession( $profileId ){
        AcritExportproplusSession::DeleteSession( $profileId, $_SERVER["DOCUMENT_ROOT"]."/bitrix/tools/acrit.exportproplus/" );
    }
    ...
    public function CalculateString( $statement ){
        if( !is_string( $statement ) ){
            throw new AriphmeticException( GetMessage( "ACRIT_EXPORTPROPLUS_CALCSTRING_WRONG_TYPE" ), 1 );
        }
            ...
            if( ( $char >= "0" ) && ( $char <= "9" ) || ( $char == "." ) ){
                ...
                if( isset( $operPriority[$char] ) ){
                    if( ")" == $char ){
                        ...
                        if( "(" != $oper ){
                            throw new AriphmeticException( GetMessage( "ACRIT_EXPORTPROPLUS_CALCSTRING_UNEXPECTED" ).' ")"', 2 );
                        }
                    }
                    else{
                ...
                elseif( strpos( " ", $char ) !== FALSE ){
                    ...
                    throw new AriphmeticException( GetMessage( "ACRIT_EXPORTPROPLUS_CALCSTRING_UNEXPECTED" ).' "'.$char.'"', 3 );
                }
            }
        }
        if( strlen( $token ) ){
        ...
        if( !empty( $operStack ) ){
            while( $oper = array_pop( $operStack ) ){
                if( "(" == $oper ){
                    throw new AriphmeticException( GetMessage( "ACRIT_EXPORTPROPLUS_CALCSTRING_UNEXPECTED" ).' "("', 4 );
                }
                array_push( $calcQueue, $oper );
            }
        }
    ...
    public function ArrayToExcel( $fileFullName, $fileTitle, $arData, $arProfile, $page ){
        if( !empty( $fileFullName ) && is_array( $arData ) ) {
            $obLog = new CAcritExportproplusLog( $arProfile["ID"] );
            try{
                $objPHPExcel = false;
                if( $page == 1 ){
                ...
                if( $objPHPExcel ){
                    $objPHPExcel->getProperties()->setCreator( "acrit.exportproplus" )
                    ->setLastModifiedBy( "acrit.exportproplus" )
                    ->setTitle( $fileTitle )
                            ...
                            foreach( $arDataset as $indexRow => $row ){
                                if( $page == 1 ){
                                    if( count( $row ) == count( $arData["HEADER"] ) ){
                                        ...
                                        $obLog->AddMessage( GetMessage( "ACRIT_EXPORTPROPLUS_PHPEXCEL_BROKEN_DATA" ).$indexRow.PHP_EOL );
                                    ...
                                    if( count( $row ) == $iHeaderColumnsCnt ){
                                        ...
                                        $obLog->AddMessage( GetMessage( "ACRIT_EXPORTPROPLUS_PHPEXCEL_BROKEN_DATA" ).( $indexRow + count( $arRows ) ).PHP_EOL );
                                        $obLog->IncProductError();
                                    }
                                }
                            }
                ...
                $obLog->AddMessage( GetMessage( "ACRIT_EXPORTPROPLUS_PHPEXCEL_BAD_EXPORT" ) );
                $obLog->IncProductError();
            }
        }
    }
...
class CAcritExportproplusStringProcess{
        ...
        if( !empty( $convertField ) ){
            if( is_array( $convertField ) ){
                foreach( $convertField as &$val ){
                    $templateValueCharset = CAcritExportproplusTools::GetStringCharset( $val );
                ...
                $templateValueCharset = CAcritExportproplusTools::GetStringCharset( $convertField );
                if( $templateValueCharset == "cp1251" ){
...
class CAcritExportproplusMarketCategories{
    ...
    private function GetLocalFilePath( $yandexCategoryServerPath ){
        ...
        $yandexCategoryLocalPath = $_SERVER["DOCUMENT_ROOT"]."/upload/acrit.exportproplus/".$yandexCategoryFileName;
    ...
    private function GetCorrectCharsetRow( $inputRow ){
        ...
        $itemCharset = CAcritExportproplusTools::GetStringCharset( $inputRow );

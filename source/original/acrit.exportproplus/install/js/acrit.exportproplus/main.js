$( function(){
    ...
    $( document ).on( 'click', '#step9 .agent-fieldset-item-delete', function(){
        ...
        $.ajax({
            ...
            'method': 'POST',
            'dataType': 'json',
            'url': '/bitrix/admin/acrit_exportproplus_ajax.php',
...
function AutofillSettingsSet( profileId ){
    ...
    $.ajax({
        ...
        'success': function( data ){
            BX.closeWait( 'waitContainer' );
            window.location = 'acrit_exportproplus_edit.php?ID=' + profileId;
        },
    });
}
...
function UpdateBitrixCloudMonitoring( addEmail ){
    ...
    $.ajax({
        ...
        'success': function( data ){
            BX.closeWait( 'waitContainer' );
            window.location = '/bitrix/admin/settings.php?mid=acrit.exportproplus';
        },
    });
}
...
function GetFbAccessToken(){
    ...
    var callback = '/bitrix/admin/acrit_exportproplus_ajax.php?ajax_action=' + 'setFbLoginUrl';
    var width = 800;
    var height = 600;

    url = '/bitrix/admin/acrit_exportproplus_ajax.php?appId=' + appId + '&appSecret=' + appSecret + '&callback=' + callback + '&scope=' + scope + '&ajax_action=' + 'getFbLogin' + '&sessid=' + BX.message( 'bitrix_sessid' );

    window.open( url, 'AcritFBAccess', 'location=yes,resizable=yes,scrollbars=yes,width=' + width + ',height=' + height + ',left=' + ( ( window.innerWidth - width ) / 2 ) + ',top=' + ( ( window.innerHeight - height ) / 2 ) );
}

...
<tr align="center">
    <td colspan="2">
        <?=BeginNote();?>
        <?=GetMessage( "DATA_EXPORTPROPLUS_VK_CATEGORY_DESCRIPTION" )?>
        <?=EndNote();?>
    </td>
</tr>
<?if( ( $arProfile["TYPE"] == "vk_trade" ) && !empty( $categories ) ){?>
                ...
                <?foreach( $categories as $cat ){?>
                    <tr>
                        ...
                        <td width="60%">
                            ...
                            <span class="field-edit" onclick="ShowMarketCategoryList( <?=$cat["ID"]?>, 'market_category_list_vk' )" style="cursor: pointer; background: #9ec710 !important;" title="<?=GetMessage( "DATA_EXPORTPROPLUS_MARKET_CATEGORY_DATA_SELECT_SECTION" );?>"></span>
                        </td>
                    </tr>
                <?}?>
            </table>
            <div id="market_category_list_vk" style="display: none">
                <input onkeyup="FilterMarketCategoryList( this, 'market_category_list_vk' )" placeholder="<?=GetMessage( "DATA_EXPORTPROPLUS_MARKET_CATEGORY_DATA_WINDOW_PLACEHOLDER" );?>">

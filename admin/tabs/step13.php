...
<tr align="center">
    <td colspan="2">
        <?=BeginNote();?>
        <?=GetMessage( "DATA_EXPORTPROPLUS_OZON_CATEGORY_DESCRIPTION" )?>
        <?=EndNote();?>
    </td>
</tr>
<tr>
    <td colspan="2" id="market_category_data_ozon">
        <table width="100%">
            <?foreach( $categories as $cat ){
                ...
                if( strlen( trim( $marketCategory[$arProfile["MARKET_CATEGORY"]["OZON"]["CATEGORY_LIST"][$cat["ID"]]]["PathName"] ) ) > 0 ){
                    ...
                    $ozonCatLocalServerPath = $_SERVER["DOCUMENT_ROOT"]."/upload/data.exportproplus/ozon_api_cat_".$marketCategory[$arProfile["MARKET_CATEGORY"]["OZON"]["CATEGORY_LIST"][$cat["ID"]]]["ProductTypeId"].".xsd";
                    $ozonCatLocalPath = "/upload/data.exportproplus/ozon_api_cat_".$marketCategory[$arProfile["MARKET_CATEGORY"]["OZON"]["CATEGORY_LIST"][$cat["ID"]]]["ProductTypeId"].".xsd";
                ...
                <tr>
                    <td width="40%">
                        ...
                        <?if( $bHasDesc ){?>
                            <span id="hint_PROFILE[CAT_<?=$cat["ID"]?>_DESC]"></span><script type="text/javascript">BX.hint_replace( BX( 'hint_PROFILE[CAT_<?=$cat["ID"]?>_DESC]' ), '<a href="<?=$ozonCatLocalPath?>" target="_blank"><?=GetMessage( "DATA_EXPORTPROPLUS_OZON_CAT_DESC_HELP" )?></a>' );</script>
                        <?}?>
                    </td>
                    <td width="60%">
                        ...
                        <span class="field-edit" onclick="ShowMarketCategoryList( <?=$cat["ID"]?>, 'market_category_list_ozon' )" style="cursor: pointer; background: #9ec710 !important;" title="<?=GetMessage( "DATA_EXPORTPROPLUS_MARKET_CATEGORY_DATA_SELECT_SECTION" );?>"></span>
                    </td>
                </tr>
            <?}?>
        </table>
        <div id="market_category_list_ozon" style="display: none">
            ...
            if( CDataExportproplusTools::ArrayValidate( $marketCategory ) ){
            ...
            <input onkeyup="FilterMarketCategoryList( this, 'market_category_list_ozon' )" placeholder="<?=GetMessage( "DATA_EXPORTPROPLUS_MARKET_CATEGORY_DATA_WINDOW_PLACEHOLDER" );?>">

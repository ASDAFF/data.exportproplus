...
if(\Bitrix\Main\Config\Option::get($moduleId, 'disable_old_core') == 'Y'){
    ...
    if( !is_array( $t ) || !isset( $t["js"] ) || !file_exists( $DOCUMENT_ROOT.$t["js"] ) ){
        $APPLICATION->ThrowException( GetMessage( "DATA_EXPORTPROPLUS_JQUERY_REQUIRE" ) );
    }
    ...
    while( $arRes = $rsData->NavNext( true, "f_" ) ){
	    ...
	    $row->AddViewField( "NAME", '<a href="data_exportproplus_edit.php?ID='.$f_ID."&amp;lang=".LANG.'" title="'.GetMessage( "parser_act_edit" ).'">'.$f_NAME."</a>" );
	    ...
	    if( $POST_RIGHT == "W" ){
		    $arActions[] = array(
			    ...
			    "ACTION" => $lAdmin->ActionRedirect( "data_exportproplus_edit.php?ID=".$f_ID )
		    );
	    }
	    ...
	    if( $POST_RIGHT == "W" ){
		    $arActions[] = array(
			    ...
			    "ACTION" => $lAdmin->ActionRedirect( "data_exportproplus_edit.php?copy=$f_ID&ID=$f_ID" )
		    );
	    }
        ...
        if( file_exists( $_SERVER["DOCUMENT_ROOT"]."/bitrix/tools/$moduleId/export_{$arRes["ID"]}_run.lock" ) ){
            if( $POST_RIGHT == "W" ){
                $arActions[] = array(
                    ...
                    "ACTION" => "UnlockExportExpress( ".$arRes["ID"]." );".$lAdmin->ActionRedirect( "data_exportproplus_list.php" )
                );
            }
        }
        ...
        if( !file_exists( $_SERVER["DOCUMENT_ROOT"]."/bitrix/tools/$moduleId/export_{$arRes["ID"]}_run.lock" )
            && ( $f_SETUP["TYPE_RUN"] == "comp" ) ){
                if( $POST_RIGHT == "W" ){
                    $runRow = "/bitrix/tools/$moduleId/data_exportproplus.php?ID=".$arRes["ID"];
        ...
        if( $POST_RIGHT == "W" ){
            $arActions[] = array(
                ...
                "ACTION" => $lAdmin->ActionRedirect( "data_exportproplus_export.php?URL_DATA_FILE_EXPORT=/upload/data_exportproplus_dump_".time().".txt&export_import=export&step=2&ID=$f_ID" )
            );
        }
    ...
    $aContext = array(
	    ...
	    array(
		    "TEXT" => GetMessage( "DATA_EXPORTPROPLUS_NEW_MIGRATE" ),
		    "LINK" => "data_exportproplus_new_migrator.php?lang=".LANG,
		    "TITLE" => GetMessage( "DATA_EXPORTPROPLUS_NEW_MIGRATE" ),
	    ),
    );
    ...
    /*if( CExportproplusInformer::CheckCRMIntergation() ){
        echo BeginNote();
        echo GetMessage( "DATA_BITRIX24_CONNECT" );
        echo EndNote();
    }*/

    echo BeginNote();
    echo GetMessage( "DATA_TIME_ZONES_DIFF_DATE" );
    echo EndNote();

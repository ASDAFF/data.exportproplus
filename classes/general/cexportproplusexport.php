...
class CDataExportproplusExport{
    private $profile;
    private $dbMan;
    static $fileExport;
    ...
    public function __construct( $profileID ){
        global $exportstep;
        $this->lockDir = $_SERVER["DOCUMENT_ROOT"]."/bitrix/tools/data.exportproplus/";
        ...
        $siteDocumentRoot = CDataExportproplusTools::GetSiteDocumentRoot( $this->profile["LID"] );
        $this->documentRoot = ( $siteDocumentRoot ) ? $siteDocumentRoot : $_SERVER["DOCUMENT_ROOT"];

        $sessionData = DataExportproplusSession::GetSession( $profileID );
        if( !$this->bApiExport ){
            $this->baseDir = $this->documentRoot."/data.exportproplus/";
            ...
            if( strlen( strstr( self::$fileExport, $this->baseDir ) ) > 0 ){
                self::$fileExport = str_replace( $this->baseDir, $this->documentRoot."/upload/data.exportproplus/", self::$fileExport );
        ...
        if( isset( $sessionData ) && !empty( $sessionData ) ){
            DataExportproplusSession::SetSession( $profileID, $sessionData );
        }
    }
    ...
    public function Export( $type = "", $cronpage = 0, $processId = 0 ){
        ...
        if( file_exists( $_SERVER["DOCUMENT_ROOT"]."/bitrix/tools/data.exportproplus/export_{$this->profile["ID"]}_run.unlock" )
            || ( $bBaseCron && file_exists( $_SERVER["DOCUMENT_ROOT"]."/bitrix/tools/data.exportproplus/export_{$this->profile["ID"]}_run.lock" ) && !$cronpage ) ){
            if( !$bBaseCron ){
                $this->Unlock();
            }
            return false;
        ...
        DataExportproplusSession::Init( $exportstep );
        ...
        $obLog = new CDataExportproplusLog( $this->profile["ID"] );
            ...
            if( $_REQUEST["unlock"] == "Y" ){
                ...
                if( $type != "cron" ){
                    ...
                    CAdminMessage::ShowMessage(
                        array(
                            "MESSAGE" => GetMessage( "DATA_EXPORTPROPLUS_PROCESS_RUN" ),
                            "TYPE" => "FAIL",
                            "HTML" => "TRUE"
                        )
                    );
                    ...
                    $subject = GetMessage( "DATA_EXPORTPROPLUS_PROCESS_RUN_SUBJECT" );
                    $errorMessage = GetMessage( "DATA_EXPORTPROPLUS_PROCESS_RUN_ERROE_MESSAGE" );
        ...
        if( CModule::IncludeModule( "catalog" ) ){
            ...
            CDataExportproplusProps::$arIBlockFilter = $obProfileUtils->PrepareIBlock( $this->profile["IBLOCK_ID"], $this->profile["USE_SKU"] );
        ...
        $elementsObj = new CDataExportproplusElement( $this->profile );
        ...
        if( !$end ){
            if( !$exportstep || $exportstep == 1 ){
                ...
                DataExportproplusSession::DeleteSession( $this->profile["ID"] );
            }
            ...
            if( $cronrun ){
                if( intval( $cronpage ) > 1 ){
                    ...
                    unlink( self::$fileExport );
                    $threads = new Threads();
                    $sessionData = DataExportproplusSession::GetSession( $this->profile["ID"] );
                    if( isset( $sessionData["EXPORTPROPLUS"]["THREAD"][$this->profile["ID"]] )
                        ...
                        DataExportproplusSession::SetSession( $this->profile["ID"] , $sessionData );
                    }
                    ...
                    for( $i = 0; $i < $steps2; $i++ ){
                        for( $j = 0; $j < $tCnt; $j++ ){
                            ...
                            $threadsId = $threads->newThread(
                                $this->documentRoot."/bitrix/modules/data.exportproplus/tools/cronrun_proc.php",
                                array(
            ...
            if( !$allPages && ( $allPages != -1 ) && !$cronrun ){
                ...
                if( $this->bApiExport ){
                    ...
                    switch( $this->profile["TYPE"] ){
                        ...
                        case "ok_trade":
                            ...
                            break;
                        case "vk_trade":
                            $this->obVkTools = new CDataExportproplusVkTools( $arUpdatedOnExportData );
                    ...
                    if( !$cronrun ){
                        LocalRedirect( "/data.exportproplus/?".$redirectLabel );
                    }
                    return;
                }
                ...
                if( ( $this->profile["SETUP"]["FILE_TYPE"] != "csv" ) && ( $this->profile["SETUP"]["FILE_TYPE"] != "xlsx" ) ){
                    ...
                    $paternCharset = CDataExportproplusTools::GetStringCharset( $basePatern );

                    if( $paternCharset == "cp1251" ){
                        $basePatern = $APPLICATION->ConvertCharset( $basePatern, "windows-1251", "UTF-8" );
                    }

                    $dateGenerate = ( $this->profile["DATEFORMAT"] == $basePatern ) ? CDataExportproplusTools::GetYandexDateTime( date( "d.m.Y H:i:s" ) ) : date( str_replace( "_", " ", $this->profile["DATEFORMAT"] ), time() );
                    ...
                    $defaultFields = array(
                        ...
                        "#CATEGORY#" => ( ( $this->profile["USE_MARKET_CATEGORY"] == "Y" ) && ( $this->profile["CHANGE_MARKET_CATEGORY"] == "Y" ) && ( $this->profile["TYPE"] != "google" ) ) ? $this->GetYandexCategoryXML( CDataExportproplusTools::GetSections( $this->profile ) ) : $this->GetCategoryXML( CDataExportproplusTools::GetSections( $this->profile ) ),
                        "#CURRENCY#" => ( CModule::IncludeModule( "catalog" ) ) ? $this->GetCurrencyXML( CDataExportproplusTools::GetCurrencies( $this->profile ) ) : "RUB",
                        "#DATE#" => $dateGenerate,
                ...
                if( CDataExportproplusTools::HasBadCronExports( $this->profile ) ){
                    $arSupportData[] = GetMessage( "DATA_EXPORTPROPLUS_PROCESS_CRON_BAD_EXPORTS" );
                }
                ...
                if( $this->profile["USE_COMPRESS"] == "Y" ){
                    ...
                    elseif( file_exists( $this->documentRoot."/upload/data.exportproplus/".$this->originalNamePath ) ){
                        $zipSavePath = $this->documentRoot."/upload/data.exportproplus/".$this->originalNamePath;
                    }
                ...
                if( $this->profile["TYPE"] == "fb_ads" ){
                    ...
                    $obFbTools = new CDataExportproplusFbTools( $this->profile );
    ...
    private function Unlock(){
        ...
        DataExportproplusSession::DeleteSession( $this->profile["ID"], $_SERVER["DOCUMENT_ROOT"]."/bitrix/tools/data.exportproplus/" );
    }
    ...
    private function GetCategoryXML( $sections ){
        ...
        foreach( $sections as $sectionId ){
            $dbSectionList = CDataExportproplusTools::GetSectionTree(
                false,
                $sectionId
            );

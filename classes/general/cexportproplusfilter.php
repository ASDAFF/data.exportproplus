...
class CDataExportproplusCatalog extends CDataCatalogCondCtrlIBlockFields{
    ...
    public static function GetControls( $strControlID = false ){
        $arControlList = array(
            "CondExportproplusSetContains" => array(
                "ID" => "CondExportproplusSetContains",
                "FIELD" => "SET_CONTAINS",
                "FIELD_TYPE" => "int",
                "MULTIPLE" => "N",
                "GROUP" => "N",
                ...
                "JS_VALUE" => array(
                    ...
                    "popup_params" => array(
                        "lang" => LANGUAGE_ID,
                        "IBLOCK_ID" => CDataExportproplusProps::$arIBlockFilter[0],
                        "discount" => "Y"
                    ),
                    "param_id" => "n"
                ),
            ),
            "CondExportproplusAvailability" => array(
                "ID" => "CondExportproplusAvailability",
        ...
        while( $arStore = $dbStore->GetNext() ){
            $arControlList["CondExportproplusStore:".$arStore["ID"]] = array(
                "ID" => "CondExportproplusStore:".$arStore["ID"],
...
class CDataExportproplusPrices extends CDataCatalogCondCtrlIBlockFields{
    public static function GetClassName(){
        return __CLASS__;
    }
...
class CDataExportproplusProps extends CDataCatalogCondCtrlIBlockProps{
...
class CDataExportproplusCatalogCond extends CDataGlobalCondTree{
    public static function GetClassName(){
        return __CLASS__;
    }
    ...
    public function OnConditionControlBuildList(){
        if( !$this->boolError && !isset( $this->arControlList ) ){
            ...
            $basicEvents = array_merge( $basicEvents, array(
                    array(
                        "TO_CLASS" => "CDataExportproplusPrices",
                        "TO_METHOD" => "GetControlDescr"
                    ),
                    array(
                        "TO_CLASS" => "CDataExportproplusCatalog",
                        "TO_METHOD" => "GetControlDescr"
                    ),
                )
            );
            foreach( $basicEvents as $arEvent ){
                if( $arEvent["TO_CLASS"] == "CDataCatalogCondCtrlIBlockProps" ){
                    $arEvent["TO_CLASS"] = "CDataExportproplusProps";
                    $arEvent["TO_MODULE_ID"] = "data.exportproplus";
                    $arEvent["TO_NAME"] = "CDataExportproplusProps::GetControlDescr (data.exportproplus)";
                }

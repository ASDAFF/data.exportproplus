...
class CDataExportproplusVkTools{
    public $profile = null;
    public $log = null;
    ...
    public function __construct( $profile ){
        ...
        $this->obVkModel = new CDataExportproplusVkModel( $this->profile );
        ...
        if( $vkMarketItemId ){
            if( intval( $this->vkAccount["GROUP_PUBLISH"] ) > 0 ){
                ...
                if( is_array( $bUpdatedMarketItem["error"] ) && !empty( $bUpdatedMarketItem["error"] ) ){
                    $this->log->AddMessage( "{$arData["NAME"]} (ID:{$arData["ID"]}) : ".str_replace( "#VK_ERROR#", "CODE: ".$bUpdatedMarketItem["error"]["error_code"]." - ".$bUpdatedMarketItem["error"]["error_msg"], GetMessage( "DATA_EXPORTPROPLUS_REQUIRED_FIELD_VK_SKIP" ) ) );
                    ...
                    foreach( $arData["VK"]["VK_MARKET_ALBUMS"] as $procVkMarketAlbumLocalId => $procVkMarketAlbum ){
                        ...
                        if( !$vkMarketAlbumId ){
                            ...
                            if( is_array( $arAddMarketAlbumResponse["error"] ) && !empty( $arAddMarketAlbumResponse["error"] ) ){
                                $this->log->AddMessage( "{$arData["NAME"]} (ID:{$arData["ID"]}) : ".str_replace( "#VK_ERROR#", "CODE: ".$arAddMarketAlbumResponse["error"]["error_code"]." - ".$arAddMarketAlbumResponse["error"]["error_msg"], GetMessage( "DATA_EXPORTPROPLUS_REQUIRED_FIELD_VK_SKIP" ) ) );
                                $this->log->IncProductError();
                            }
                            else{
                                ...
                                if( is_array( $bAddMarketItemToAlbums["error"] ) && !empty( $bAddMarketItemToAlbums["error"] ) ){
                                    $this->log->AddMessage( "{$arData["NAME"]} (ID:{$arData["ID"]}) : ".str_replace( "#VK_ERROR#", "CODE: ".$bAddMarketItemToAlbums["error"]["error_code"]." - ".$bAddMarketItemToAlbums["error"]["error_msg"], GetMessage( "DATA_EXPORTPROPLUS_REQUIRED_FIELD_VK_SKIP" ) ) );
                                    $this->log->IncProductError();
                                }
                            }
                        }
                            ...
                            if( is_array( $bUpdatedMarketAlbumItem["error"] ) && !empty( $bUpdatedMarketAlbumItem["error"] ) ){
                                $this->log->AddMessage( "{$arData["NAME"]} (ID:{$arData["ID"]}) : ".str_replace( "#VK_ERROR#", "CODE 777: ".$bUpdatedMarketAlbumItem["error"]["error_code"]." - ".$bUpdatedMarketAlbumItem["error"]["error_msg"], GetMessage( "DATA_EXPORTPROPLUS_REQUIRED_FIELD_VK_SKIP" ) ) );
                                $this->log->IncProductError();
                            }
                        }
                    ...
                    foreach( $arPhotoIds as $vkPhotoId ){
                        ...
                        if( is_array( $bUpdatedGroupPhoto["error"] ) && !empty( $bUpdatedGroupPhoto["error"] ) ){
                            $this->log->AddMessage( "{$arData["NAME"]} (ID:{$arData["ID"]}) : ".str_replace( "#VK_ERROR#", "CODE: ".$bUpdatedGroupPhoto["error"]["error_code"]." - ".$bUpdatedGroupPhoto["error"]["error_msg"], GetMessage( "DATA_EXPORTPROPLUS_REQUIRED_FIELD_VK_SKIP" ) ) );
                            $this->log->IncProductError();
                        }
                    }
                    ...
                    foreach( $arData["VK"]["VK_ALBUMS"] as $vkMultiAlbumItemIndex => $vkMultiAlbumItem ){
                        ...
                        if( !$vkPhotoAlbumId ){
                            ...
                            if( is_array( $arCreatedGroupAlbum["error"] ) && !empty( $arCreatedGroupAlbum["error"] ) ){
                                $this->log->AddMessage( "{$arData["NAME"]} (ID:{$arData["ID"]}) : ".str_replace( "#VK_ERROR#", "CODE: ".$arCreatedGroupAlbum["error"]["error_code"]." - ".$arCreatedGroupAlbum["error"]["error_msg"], GetMessage( "DATA_EXPORTPROPLUS_REQUIRED_FIELD_VK_SKIP" ) ) );
                                $this->log->IncProductError();
                            }
                            else{
                                ...
                                if( !$bHasPhotoItem ){
                                    ...
                                    if( is_array( $arAddedGroupPhoto["error"] ) && !empty( $arAddedGroupPhoto["error"] ) ){
                                        $this->log->AddMessage( "{$arData["NAME"]} (ID:{$arData["ID"]}) : ".str_replace( "#VK_ERROR#", "CODE: ".$arAddedGroupPhoto["error"]["error_code"]." - ".$arAddedGroupPhoto["error"]["error_msg"], GetMessage( "DATA_EXPORTPROPLUS_REQUIRED_FIELD_VK_SKIP" ) ) );
                                        $this->log->IncProductError();
                                    }
                                    else{
                            ...
                            if( is_array( $bUpdatedGroupAlbum["error"] ) && !empty( $bUpdatedGroupAlbum["error"] ) ){
                                $this->log->AddMessage( "{$arData["NAME"]} (ID:{$arData["ID"]}) : ".str_replace( "#VK_ERROR#", "CODE: ".$bUpdatedGroupAlbum["error"]["error_code"]." - ".$bUpdatedGroupAlbum["error"]["error_msg"], GetMessage( "DATA_EXPORTPROPLUS_REQUIRED_FIELD_VK_SKIP" ) ) );
                                $this->log->IncProductError();
                            }
                        }
                    }
            ...
            if( intval( $this->vkAccount["GROUP_PUBLISH"] ) > 0 ){
                ...
                if( is_array( $arAddMarketItemResponse["error"] ) && !empty( $arAddMarketItemResponse["error"] ) ){
                    $this->log->AddMessage( "{$arData["NAME"]} (ID:{$arData["ID"]}) : ".str_replace( "#VK_ERROR#", "CODE: ".$arAddMarketItemResponse["error"]["error_code"]." - ".$arAddMarketItemResponse["error"]["error_msg"], GetMessage( "DATA_EXPORTPROPLUS_REQUIRED_FIELD_VK_SKIP" ) ) );
                    $this->log->IncProductError();
                }
                else{
        ...
        if( $vkWallPostId ){
            if( intval( $this->vkAccount["USER_PUBLISH"] ) > 0 ){
                ...
                if( is_array( $bUpdatedWallItem["error"] ) && !empty( $bUpdatedWallItem["error"] ) ){
                    $this->log->AddMessage( "{$arData["NAME"]} (ID:{$arData["ID"]}) : ".str_replace( "#VK_ERROR#", "CODE: ".$bUpdatedWallItem["error"]["error_code"]." - ".$bUpdatedWallItem["error"]["error_msg"], GetMessage( "DATA_EXPORTPROPLUS_REQUIRED_FIELD_VK_SKIP" ) ) );
                    $this->log->IncProductError();
                }
            }
        }
            ...
            if( intval( $this->vkAccount["USER_PUBLISH"] ) > 0 ){
                ...
                if( is_array( $arAddWallItemResponse["error"] ) && !empty( $arAddWallItemResponse["error"] ) ){
                    $this->log->AddMessage( "{$arData["NAME"]} (ID:{$arData["ID"]}) : ".str_replace( "#VK_ERROR#", "CODE: ".$arAddWallItemResponse["error"]["error_code"]." - ".$arAddWallItemResponse["error"]["error_msg"], GetMessage( "DATA_EXPORTPROPLUS_REQUIRED_FIELD_VK_SKIP" ) ) );
                    $this->log->IncProductError();
                }
    ...
    public function SyncDataDiver(){
            ...
            if( !isset( $this->profile["VK"]["VK_NEW_RELATIONS"]["MARKET_ITEMS"][$localMarketItemId] ) ){
                foreach( $this->profile["VK"]["VK_RELATIONS"]["PHOTO_ITEMS"][$localMarketItemId] as $vkPhotoIndex => $vkPhotoId ){
                    ...
                    if( is_array( $arDeleteGroupPhotoResponse["error"] ) && !empty( $arDeleteGroupPhotoResponse["error"] ) ){
                        $this->log->AddMessage( "{$arData["NAME"]} (ID:{$arData["ID"]}) : ".str_replace( "#VK_ERROR#", "CODE: ".$arDeleteGroupPhotoResponse["error"]["error_code"]." - ".$arDeleteGroupPhotoResponse["error"]["error_msg"], GetMessage( "DATA_EXPORTPROPLUS_REQUIRED_FIELD_VK_SKIP" ) ) );
                        $this->log->IncProductError();
                    }
                    else{
                ...
                if( is_array( $arDeleteResponse["error"] ) && !empty( $arDeleteResponse["error"] ) ){
                    $this->log->AddMessage( "{$arData["NAME"]} (ID:{$arData["ID"]}) : ".str_replace( "#VK_ERROR#", "CODE: ".$arDeleteResponse["error"]["error_code"]." - ".$arDeleteResponse["error"]["error_msg"], GetMessage( "DATA_EXPORTPROPLUS_REQUIRED_FIELD_VK_SKIP" ) ) );
                    $this->log->IncProductError();
                }
                else{
        ...
        foreach( $this->profile["VK"]["VK_RELATIONS"]["PHOTO_ALBUMS"] as $localPhotoAlbumId => $vkPhotoAlbumId ){
            ...
            if( is_array( $arGetGroupPhotosResponse["error"] ) && !empty( $arGetGroupPhotosResponse["error"] ) ){
                $this->log->AddMessage( "{$arData["NAME"]} (ID:{$arData["ID"]}) : ".str_replace( "#VK_ERROR#", "CODE: ".$arGetGroupPhotosResponse["error"]["error_code"]." - ".$arGetGroupPhotosResponse["error"]["error_msg"], GetMessage( "DATA_EXPORTPROPLUS_REQUIRED_FIELD_VK_SKIP" ) ) );
                $this->log->IncProductError();
            }
            else{
                if( empty( $arGetGroupPhotosResponse["response"]["items"] ) ){
                    ...
                    if( is_array( $arDeleteGroupAlbumResponse["error"] ) && !empty( $arDeleteGroupAlbumResponse["error"] ) ){
                        $this->log->AddMessage( "{$arData["NAME"]} (ID:{$arData["ID"]}) : ".str_replace( "#VK_ERROR#", "CODE: ".$arDeleteGroupAlbumResponse["error"]["error_code"]." - ".$arDeleteGroupAlbumResponse["error"]["error_msg"], GetMessage( "DATA_EXPORTPROPLUS_REQUIRED_FIELD_VK_SKIP" ) ) );
                        $this->log->IncProductError();
                    }
                    else{
        ...
        foreach( $this->profile["VK"]["VK_RELATIONS"]["MARKET_ALBUMS"] as $localMarketAlbumId => $vkMarketAlbumId ){
            ...
            if( is_array( $arGetMarketItemsResponse["error"] ) && !empty( $arGetMarketItemsResponse["error"] ) ){
                $this->log->AddMessage( "{$arData["NAME"]} (ID:{$arData["ID"]}) : ".str_replace( "#VK_ERROR#", "CODE: ".$arGetMarketItemsResponse["error"]["error_code"]." - ".$arGetMarketItemsResponse["error"]["error_msg"], GetMessage( "DATA_EXPORTPROPLUS_REQUIRED_FIELD_VK_SKIP" ) ) );
                $this->log->IncProductError();
            }
            else{
                if( intval( $arGetMarketItemsResponse["response"]["count"] ) <= 0 ){
                    ...
                    if( is_array( $arDeleteMarketAlbumResponse["error"] ) && !empty( $arDeleteMarketAlbumResponse["error"] ) ){
                        $this->log->AddMessage( "{$arData["NAME"]} (ID:{$arData["ID"]}) : ".str_replace( "#VK_ERROR#", "CODE: ".$arDeleteMarketAlbumResponse["error"]["error_code"]." - ".$arDeleteMarketAlbumResponse["error"]["error_msg"], GetMessage( "DATA_EXPORTPROPLUS_REQUIRED_FIELD_VK_SKIP" ) ) );
                        $this->log->IncProductError();
                    }
                    else{
    ...
    public function DeleteAllProductAlbums(){
        ...
        CDataExportproplusLog::DataDump( $arVkAllProductAlbumsResponse );
        if( intval( $arVkAllProductAlbumsResponse["response"]["count"] ) > 0 ){
            foreach( $arVkAllProductAlbumsResponse["response"]["items"] as $iProductAlbumIndex => $arVkProductAlbum ){

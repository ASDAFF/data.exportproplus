...
class CDataExportproplusOkTools{
    public $profile = null;
    public $log = null;
    ...
    public function __construct( $profile ){
        ...
        $this->obOkModel = new CDataExportproplusOk( $this->profile );
        ...
        if( !$okMediatopicId ){
            if( intval( $this->obOkModel->okAccount["GROUP"] ) > 0 ){
                ...
                if( isset( $arAddMediatopicResponse["error_code"] ) ){
                    $this->log->AddMessage( "{$arData["NAME"]} (ID:{$arData["ID"]}) : ".str_replace( "#OK_ERROR#", "CODE: ".$arAddMediatopicResponse["error_code"]." - ".$arAddMediatopicResponse["error_msg"], GetMessage( "DATA_EXPORTPROPLUS_REQUIRED_FIELD_OK_SKIP" ) ) );
                        ...
                        foreach( $arData["OK"]["OK_ALBUMS"] as $okMultiAlbumItemIndex => $okMultiAlbumItem ){
                            ...
                            if( !$okPhotoAlbumId ){
                                ...
                                if( isset( $arCreatedGroupAlbum["error_code"] ) ){
                                    $this->log->AddMessage( "{$arData["NAME"]} (ID:{$arData["ID"]}) : ".str_replace( "#OK_ERROR#", "CODE: ".$arCreatedGroupAlbum["error_code"]." - ".$arCreatedGroupAlbum["error_msg"], GetMessage( "DATA_EXPORTPROPLUS_REQUIRED_FIELD_OK_SKIP" ) ) );
                                    $this->log->IncProductError();
                                }
                                else{
                                ...
                                if( !$bHasPhotoItem ){
                                    ...
                                    if( isset( $arAddedGroupPhoto["error_code"] ) ){
                                        $this->log->AddMessage( "{$arData["NAME"]} (ID:{$arData["ID"]}) : ".str_replace( "#OK_ERROR#", "CODE: ".$arAddedGroupPhoto["error_code"]." - ".$arAddedGroupPhoto["error_msg"], GetMessage( "DATA_EXPORTPROPLUS_REQUIRED_FIELD_OK_SKIP" ) ) );
                                        $this->log->IncProductError();
                                    }
                                    else{
                        ...
                        foreach( $arData["OK"]["OK_CATALOGS"] as $okMultiCatalogItemIndex => $okMultiCatalogItem ){
                            ...
                            if( !$okMarketCatalogId ){
                                ...
                                if( isset( $arAddMarketCatalog["error_code"] ) ){
                                    $this->log->AddMessage( "{$arData["NAME"]} (ID:{$arData["ID"]}) : ".str_replace( "#OK_ERROR#", "CODE: ".$arAddMarketCatalog["error_code"]." - ".$arAddMarketCatalog["error_msg"], GetMessage( "DATA_EXPORTPROPLUS_REQUIRED_FIELD_OK_SKIP" ) ) );
                                    $this->log->IncProductError();
                                }
                                else{
                                ...
                                if( !$bHasItemInCatalog ){
                                    ...
                                    if( isset( $arAddedMarketItem["error_code"] ) ){
                                        $this->log->AddMessage( "{$arData["NAME"]} (ID:{$arData["ID"]}) : ".str_replace( "#OK_ERROR#", "CODE: ".$arAddedMarketItem["error_code"]." - ".$arAddedMarketItem["error_msg"], GetMessage( "DATA_EXPORTPROPLUS_REQUIRED_FIELD_OK_SKIP" ) ) );
                                        $this->log->IncProductError();
                                    }
                                    else{
